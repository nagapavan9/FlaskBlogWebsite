...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            #lands on  account page after logging in
...
def account():
    ...
    # This will auto populate the current user details in  the update username and update email fields
...
def update_post(post_id):
    ...
    if post.author != current_user:
        # 403 is a http response for FORBIDDEN route
        abort(403)
    form = PostForm()
    if form.validate_on_submit():
...
def send_reset_email(user):
    ...
    msg = Message('Password Request Reset', sender='noreply@demo.com', recipients=[user.email])
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password has been updated! You may now able to login !', 'success')
        return redirect(url_for('login'))
